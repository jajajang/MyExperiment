...
import my_model_2dim_todevice as my_modell
...
def main():
    ...
    crit2 = my_modell.myLoss().to(device)
    criterion_ = my_modell.myLossA().to(device)
    criterion2_ = my_modell.myLossA().to(device)
    criterionC_ = my_modell.myLossC().to(device)
    ...
    for i, datata in enumerate(train_loader):
        ...
        if (epoch%100>90) or (epoch%100<10):
            ...
            if i % args.print_freq ==0:
                ...
                'Top5 {top5.val:.3f}({top5.avg:.3f})\t'.format(top1=top1, top5=top5))
